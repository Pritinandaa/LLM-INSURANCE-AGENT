...
@router.post(
    ...
    description="Submit a broker email to generate an insurance quote"
)
async def process_quote(request: QuoteRequest):
    """
    Process a broker email and generate an insurance quote.

    The email will be processed through the 10-step underwriting pipeline:
    1. Email parsing
...
@router.post(
    ...
    description="Upload an email file (.txt or .eml) to generate a quote"
)
...
@router.post(
    "/api/agents/analyze",
    tags=["Agents"],
    summary="Direct Llama Agent Analysis",
    description="Run the expert Llama agents directly on a broker email and see the full extraction and reasoning."
)
async def analyze_with_agents(request: QuoteRequest):
    """
    Directly invoke the Broker and Underwriter agents on NVIDIA NIM.
    Returns the raw agent outputs for debugging and verification.
    """
    try:
        orchestrator = UnderwritingAgentOrchestrator()
        results = await orchestrator.run_workflow(request.email_content)
        return {
            "success": True,
            "results": results
        }
